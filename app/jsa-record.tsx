...
export default function JsaRecord(){const {id}=useLocalSearchParams<{id:string}>(),router=useRouter();const [record,setRecord]=useState<any>(null),[error,setError]=useState(''),[read,setRead]=useState(true),[printing,setPrinting]=useState(false);const returnToPrint=useRef(false);
 ...
 {read?<View style={{flex:1}}><View style={{flexDirection:"row",justifyContent:"space-between",flexWrap:"wrap"}}><TouchableOpacity accessibilityRole="button" onPress={()=>setRead(false)} style={{padding:16,minHeight:48}}><Text style={{color:"#99710c",fontWeight:"700"}}>JSA details</Text></TouchableOpacity>{canAddLocation&&<TouchableOpacity accessibilityRole="button" onPress={()=>router.push({pathname:"/add-location",params:{id}} as any)} style={{padding:16,minHeight:48}}><Text style={{color:"#99710c",fontWeight:"700"}}>＋ Add location</Text></TouchableOpacity>}</View><WebView source={{html:jsaDocumentHtml(record)}} javaScriptEnabled={false} style={{flex:1}}/></View>:<ScrollView contentContainerStyle={{padding:18,gap:16}}><Text style={{fontSize:22,fontWeight:'700'}}>{record.state==='closed'?'Closed JSA':record.state==='open'?'Ongoing JSA':'Saved JSA'}</Text><View><JsaSummaryCard driverName={record.driverName} truckNumber={record.truckNumber} date={record.date} signedAt={signingTime(record.timestamp)} rows={rows} onAddLocation={canAddLocation?()=>router.push({pathname:'/add-location',params:{id}} as any):undefined}/></View><TouchableOpacity accessibilityRole="button" onPress={()=>setRead(true)} style={{minHeight:48,justifyContent:"center"}}><Text style={{color:"#99710c",fontWeight:"700"}}>Read this JSA</Text></TouchableOpacity>{(record.additions||[]).map((a:any,i:number)=><View key={a.id||i} style={{borderTopWidth:1,borderColor:"#ddd",paddingTop:8}}><Text style={{fontWeight:"700"}}>Added: {a.location}</Text><Text>{a.activity} · {signingTime(a.acknowledgedAtMs)}</Text></View>)}</ScrollView>}
