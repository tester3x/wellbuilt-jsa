...
export default function OpenJsas(){const router=useRouter(),{t}=useLanguage(),[rows,setRows]=useState<any[]>([]),[error,setError]=useState(''),[busy,setBusy]=useState(false),[loading,setLoading]=useState(true);
 ...
 </ScrollView><BottomActionBar><TouchableOpacity accessibilityRole="button" style={{width:88,minHeight:52,alignItems:"center",justifyContent:"center"}} onPress={()=>router.push("/(tabs)/history" as any)}><Ionicons name="time-outline" size={25}/><Text style={{fontSize:12}}>{t('Saved JSAs')}</Text></TouchableOpacity><TouchableOpacity accessibilityRole="button" style={{width:88,minHeight:52,alignItems:"center",justifyContent:"center"}} onPress={()=>router.push("/(tabs)" as any)}><Ionicons name="home-outline" size={25} color="#DAA520"/><Text style={{fontSize:12}}>{t('Job Details')}</Text></TouchableOpacity><MoreMenu placement="bottom"/></BottomActionBar></View>;
}
