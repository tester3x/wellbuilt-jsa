...
export default function OpenJsas(){const router=useRouter(),[rows,setRows]=useState<any[]>([]),[error,setError]=useState(''),[busy,setBusy]=useState(false),[loading,setLoading]=useState(true);
 ...
 {loading&&<Text>Checking open JSAs…</Text>}
 {!loading&&!error&&!rows.length&&<Text>No open JSAs.</Text>}
 {!!error&&<TouchableOpacity disabled={loading} onPress={()=>void load()}><Text>Retry loading JSAs</Text></TouchableOpacity>}
