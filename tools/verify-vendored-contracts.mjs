...
// ── the package is actually consumed by this app ─────────────────────────
// Vendoring must not quietly orphan the dependency: if nothing imports it,
// a broken pin would pass every check above and still ship.
{
  const importers = [];
  const walk = (d) => {
    for (const e of readdirSync(d, { withFileTypes: true })) {
      ...
      if (readFileSync(p, 'utf8').includes(PKG)) importers.push(e.name);
    }
  };
  walk(ROOT);
  check('the vendored package is imported by application code', importers.length > 0,
