...
export default function PrinterSettings() {
  ...
  const update = async (patch: Partial<Settings>, closeDevices = false) => {
    ...
    generation.current++;
    setBusy(true);
    try { const next = await updatePrinter(patch); setP(next); if (closeDevices) setDevices([]); setMessage(`Selected printer: ${next.name || 'none'}`); }
  ...
  return <View style={s.card}>
    ...
    {devices.map(d=><Pressable key={d.macAddress} disabled={busy} style={[s.choice,p.macAddress===d.macAddress&&s.selected]} onPress={()=>void update({name:d.name,macAddress:d.macAddress},true)}><Text style={s.text}>{d.name || 'Bluetooth device'}</Text><Text style={s.hint}>{d.macAddress}</Text></Pressable>)}
    ...
    <Text style={s.hint}>Choose Regular printer when printing a JSA. Android’s print dialog handles HP Print Service and other installed printer services.</Text>
