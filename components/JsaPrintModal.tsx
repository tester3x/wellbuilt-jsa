...
export default function JsaPrintModal({visible,record,onClose,onSettings}:{visible:boolean;record:any;onClose:()=>void;onSettings:()=>void}){
 ...
 const run=async(thermal:boolean)=>{if(lock.current)return;lock.current=true;setBusy(true);try{
  if(thermal){const current=await refreshPrinter();setPrinter(current.printer);if(!current.printer.macAddress)throw new Error('Select a paired thermal printer in Settings.');
 ...
 }catch(e){setStatus(e instanceof Error?e.message:String(e));}finally{lock.current=false;setBusy(false);}};
