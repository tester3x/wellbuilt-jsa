...
function locationSections(r:any){
 ...
 const signedAt=signingTime(r.timestamp||r.createdAt);
 const rows=[...originals.map(item=>({...item,meta:`Original · ${signedAt}`,different:false})),...additions.map((a:any)=>({location:a.location,activity:a.activity,meta:`Added · ${signingTime(a.acknowledgedAtMs)}`,different:additionHasDifferences(a)}))];
 const covered=rows.length?`<section class="location-block"><h2>Locations covered</h2>${rows.map(row=>`<div class="location-row"><div><b>${row.different?'* ':''}${escape(row.location)}</b><span>${escape(row.activity||'—')}</span></div><small>${escape(row.meta)}</small></div>`).join('')}${additions.length?'<p class="location-note">Added locations without an * were acknowledged under the same company assessment. Location-specific differences are listed separately.</p>':''}</section>`:'';
 const changed=additions.filter(additionHasDifferences);
 const differences=changed.length?`<section class="location-block"><h2>Location-specific differences</h2>${changed.map((a:any)=>`<div class="difference"><h3>* ${escape(a.location)} · ${escape(a.activity)}</h3>${a.conditionsDiffer===true||a.conditionsDiffer===undefined?`<div><b>Additional hazards:</b> ${escape(a.hazards)}</div><div><b>Additional controls:</b> ${escape(a.controls)}</div>${a.ppe?`<div><b>PPE changes:</b> ${escape(a.ppe)}</div>`:''}`:''}${a.taskAssessment?`<h3>Additional task assessment</h3>${(a.taskAssessment.templates||[]).map((t:any)=>`<div>${escape(t.name)} · Version ${escape(t.version)}</div>`).join('')}${(a.taskAssessment.steps||[]).map((s:any)=>`<h3>${escape(s.title)}</h3>${(s.items||[]).map((i:any)=>`<div class="item"><b>Hazard:</b> ${escape(i.hazard)}<br><b>Controls:</b> ${escape(i.controls)}</div>`).join('')}`).join('')}${(a.taskAssessment.ppeItems||[]).length?`<div><b>PPE:</b> ${(a.taskAssessment.ppeItems||[]).map((p:any)=>escape(p.label)).join(', ')}</div>`:''}${(a.taskAssessment.preparedItems||[]).length?`<div><b>Preparation:</b> ${(a.taskAssessment.preparedItems||[]).map((p:any)=>escape(p.label)).join(', ')}</div>`:''}`:''}<small>Acknowledged ${escape(signingTime(a.acknowledgedAtMs))}</small></div>`).join('')}</section>`:'';
...
export function jsaDocumentHtml(r:any,width?:3|4):string{
 ...
 const blocks=locationSections(r);
