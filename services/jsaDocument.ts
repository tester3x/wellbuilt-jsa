...
export function jsaDocumentHtml(r:any,width?:3|4):string{
 ...
 h1{font-size:20px}h2{font-size:16px;margin:14px 0 5px}h3{font-size:14px;margin:9px 0 3px}.row{display:flex;justify-content:space-between;gap:12px;border-bottom:1px solid #aaa;padding:4px 0}.item{break-inside:avoid;margin:5px 0}img{max-width:100%;width:240px;height:70px;object-fit:contain}.note{border:1px solid;padding:7px}.footer{margin-top:15px;border-top:1px solid;padding-top:6px}
 ...
 <h2>Original signature</h2>${image?`<img src="${image}" alt="Driver signature">`:''}<div>${escape(r.driverLegalName||r.signature||r.driverName)}</div><div>Signed: ${escape(signingTime(r.timestamp||r.createdAt))}</div>
