...
export function jsaDocumentHtml(r:any,width?:3|4):string{
 ...
 <section class="signature"><h2>Original signature</h2>${image?`<img src="${image}" alt="Driver signature">`:''}<div>${escape(r.driverLegalName||r.signature||r.driverName)}</div><div>Signed: ${escape(signingTime(r.timestamp||r.createdAt))}</div></section>
