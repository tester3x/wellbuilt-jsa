// ============================================================
// WB Diagnostics helper — WB JSA (Phase 2)
//
// See wellbuilt-tickets/utils/wbDiagLog.ts for full notes — this
// is the WB JSA copy with `app: 'wbjsa'`. Identical shape and
// safety contract: fire-and-forget, sanitized, kill-switchable,
...
async function getDiagnosticIdToken(): Promise<string | null> {
  try {
    const { awaitGovernedAuthReady, getGovernedAuth } = await import('./sso/jsaGovernedAuthLive');
    await awaitGovernedAuthReady();
    const user = getGovernedAuth().currentUser;
    if (!user) return null;
    try {
